...
export default class Slider extends PureComponent {
  ...
  render() {
    ...
    var {value, buffer, containerSize, trackSize, thumbSize, allMeasured} = this.state;
    ...
    var bufferTrackStyle = {
        position: 'absolute',
        width: containerSize.width * (this.props.buffer / maximumValue) - containerSize.width * (this._getCurrentValue() / maximumValue),
        backgroundColor: bufferTrackTintColor,
        transform: [
            { translateX: containerSize.width * (this._getCurrentValue() / maximumValue)},
            { translateY: 0 }
        ],
        ...valueVisibleStyle
    };
  ...
  _setCurrentValueAndBuffer = (_value,_buffer) => {
    this.state.value.setValue(_value);
    // this.state.buffer.setValue(_buffer);
  };
