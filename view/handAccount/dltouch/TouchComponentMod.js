...
//mod 包裹的touch层 现在暂时存放的是日期   以后可以装模块

//所有的实现 参照img  都是一样的代码和实现
export default class TouchComponentMod extends Component {
  ...
  getMod = ()=>{
    var mode = {
      ...
      dateMod:this.strMod.dateMod,
      datec:this.strMod.datec,
      scale:this._child.getHeight().scale
    }
    return mode
  }
  ...
  componentDidMount(){
      this.setState({
        height:this._child.getHeight().height,
        width:this._child.getHeight().width,
      })
  }
  ...
  _onPanResponderMove = (evt, gestureState)=>{
   ...
   //move
    if (evt.nativeEvent.changedTouches.length <= 1) {
        ...
        var htop = Dimensions.get('window').height*0.08
        var htopEnd = Dimensions.get('window').height*0.8
        var hwidth = 0
  ...
  getLenbyEvent = (event) =>{
    ...
    var py=event.nativeEvent.pageY - Dimensions.get('window').height*0.08
  ...
  _onTouchMove = (event)=>{
      ...
      this._child.toSetHeightAndWidth(newh,neww,sc)
      this.setState({
        width:neww,
        height:newh,
        angle:angle,
